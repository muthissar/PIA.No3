...
class Config:
    ...
    def __post_init__(self):
        ...
        args = dict(**self.weight.__dict__, ts=self.ts, k_traces=self.k_traces)
        args_str = slugify(str(tuple(sorted(args.items()))))
        self.out = Path(f'out/{args_str}')
        # NOTE: interpret as range
        if isinstance(self.ts, Tuple) and len(self.ts) == 3:
            self.ts = torch.arange(*self.ts)
        else:
            raise ValueError(f"ts must be tuple, got {type(self.ts)}")
...
def main(c :Config):
    ...
    for path in tqdm.tqdm(c.pieces, desc='Pieces completed', disable=rank != 0, position=0):
        piece_folder = c.out.joinpath(Path(path).stem)
        ...
        for i in tqdm.tqdm(np.arange(rank, c.samples_per_template, world_size), desc='sample pr piece', disable=rank != 0, position=1):
            ...
            fresh = dataloader_generator.dataset.process_score('/share/home/mathias/.cache/mutdata/pia/databases/Piano/transcriptions/midi/Bach, Carl Philipp Emanuel, Keyboard Sonata in F major, H.55, zRPTf8eGLWc.mid')
            toks = dataloader_generator.dataset.tokenize(fresh)
            _, tensors = zip(*toks.items())
            tensor = torch.stack([torch.tensor(t) for t in tensors],dim=1)
            # NOTE: debugging end
            ...
            (
                ...
                x=x.clone(),
                interpolation_time_points=c.ts,
                k_traces=c.k_traces,
                weight_fn=c.weight,
...
def plot(c : Config):
    ...
    for song_dir in [f for f in c.out.glob('*/') if f.is_dir()]:
        ...
        # TODO: deprecate patch ######
        if res_temp.tok.dim() == 2:
            toks = res_temp.tok[None]
        else:
            toks = res_temp.tok
        res_temp.decoding_end = 731
        ### patch end
        temp_tok = data_processor.postprocess(toks, res_temp.decoding_end, metadata_dict)
        # HACK: we hardcore start 3 since the original sequence is padded... We should have the full piece, and not -3.
        s = temp_tok[0]
        temp_midi = temp_file.parent.joinpath(f'song.mid')
        dataloader_generator.write(s, temp_midi.parent.joinpath(temp_midi.stem))
        for sample in song_dir.rglob('*/ic.pt'):
            if sample == temp_file:
                continue
            res_gen = ICRes.load(p=sample)
            sample_tok = data_processor.postprocess(res_gen.tok[None], res_gen.decoding_end-1, metadata_dict)
            # HACK: we hardcore start 3 since the original sequence is padded... We should have the full piece, and not -3.
            s = sample_tok[0]
            ...
            for i, (f, r) in enumerate(zip(files, res)):
                midi2 = pretty_midi.PrettyMIDI(str(f))
                ...
                unique_timepoints_, cum_ics = unique_timepoints(r.timepoints, r.ic_tok[metadata_dict['decoding_start']:r.decoding_end])
                ...
                ax[i*figs_pr_sample+2].step(c.ts, r.ic_int[:, channels], where='post')
                ax[i*figs_pr_sample+2].legend(legends[channels])

                ax[i*figs_pr_sample+3].step(c.ts, r.ic_int[:, channels].sum(1), where='post')
...
if __name__ == "__main__":
    ...
    if args.subcommand == "main":
        if 'RANK' not in  os.environ or int(os.environ['RANK']) == 0 :
            ...
            parser.save(args, dir.joinpath('config.yaml'))
        main(c)
